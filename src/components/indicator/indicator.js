...
const Indicator = memo(props => {
  ...
  useEffect(() => {
    if (props.index === 0) {
      ...
      //滚到尾
      indicatorRef.current.style.transform = `translate(-${indicatorRef.current.scrollWidth - indicatorRef.current.clientWidth}px)`
    } else {
      indicatorScroll()
    }
  }, [props.index])
  ...
  const indicatorScroll = () => {
    ...
    //下一个子元素宽度
    const nextChildWidth = indicatorRef.current.children[props.index].clientWidth
    //下一个子元素到最左边需要滚动的距离
